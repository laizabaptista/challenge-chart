...
function verifyTime(tInterval, tCurrent) {
...
function updateDatas(newInterval, chartData1) {
  for (let i = 0; i < chartData1.length; i++) {
    if (!verifyTime(newInterval, chartData1[i][0])) {
      chartData1.splice(i, 1);
    }
  }
}

function labelCategorie(sJson, group1, select1, timestamp1, chartData1) {
  let label = "";
  let labelgroup = [];
...
export function pDataChart(str) {
  let array = createJson(str);
  ...
  for (let i = index + 1; i < array.length; i++) {
    switch (array[i].type) {
      ...
      case "data":
        if (dict["span"] && verifyTime(tGlobal, array[i].timestamp)) {
          labelCategorie(
            array[i],
            group,
            select,
